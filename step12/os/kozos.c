...
thread_run(kz_func_t func, char *name, int priority,
		    int stacksize, int argc, char *argv[])
{
	...
	*(--sp) = (uint32)thread_end;
	...
	/*
	 * In the case of priority is zero,
	 *   to start with interrupts disabled.
	 */
...
static int
thread_wakeup(kz_thread_id_t id)
{
	putcurrent();
...
static void
sendmsg(kz_msgbox *mboxp, kz_thread *thp, int size, char *p)
{
	kz_msgbuf *mp;
...
static int
thread_setintr(softvec_type_t type, kz_handler_t handler)
{
...
static void
srvcall_proc(kz_syscall_type_t type, kz_syscall_param_t *p)
{
	current = NULL;
	call_functions(type, p);
}
...
void
kz_start(kz_func_t func, char *name, int priority,
		    int stacksize, int argc, char *argv[])
{
	...
	current = (kz_thread *)thread_run(func, name, priority,
		    stacksize, argc, argv);
...
void
kz_srvcall(kz_syscall_type_t type, kz_syscall_param_t *param)
{
